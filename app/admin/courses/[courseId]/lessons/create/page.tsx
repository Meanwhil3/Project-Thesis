...
export default function CreateLessonPage() {
  ...
  const [attachments, setAttachments] = useState<{ id: string, name: string, type: string, size: string, file: File }[]>([]);
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      body.append('videoUrls', JSON.stringify(videoList.map(v => v.url)));
      attachments.forEach(at => body.append('files', at.file, at.name));
    ...
    <div className="min-h-screen font-kanit">
      <main className="mx-auto max-w-6xl px-4 pb-16 pt-10">
        ...
        <div className="mb-7">
          ...
          <p className="text-sm text-[#6E8E59]">สร้างเนื้อหาบทเรียนสำหรับคอร์สอบรม</p>
        </div>
                  ...
                  <div key={file.id} className="flex items-center justify-between rounded-xl border border-[#CDE3BD] bg-white p-3 shadow-sm">
                    <div className="flex items-center gap-3 truncate">
                      ...
                      <div className="truncate">
                        ...
                        <p className="text-[11px] text-[#6E8E59]">{file.size}</p>
                      </div>
                    </div>
